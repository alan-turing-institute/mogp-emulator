...
def matern_5_2(x1, x2, params):
    ...
    K = calc_r(x1, x2, params)
    K = np.exp(params[D - 1]) * (1. + np.sqrt(5.)*K + 5./3.*K**2)*np.exp(-np.sqrt(5.) * K)
...
def matern_5_2_deriv(x1, x2, params):
    ...
    r = calc_r(x1, x2, params)
    
    for d in range(D - 1):
        dKdtheta[d] = 0.5 * np.exp(params[d]) * cdist(np.reshape(x1[:,d], (n1, 1)),
                                                      np.reshape(x2[:,d], (n2, 1)), "sqeuclidean")
        dKdtheta[d] *= -5./3.*np.exp(params[D-1])*(1. + np.sqrt(5.)*r) * np.exp(-np.sqrt(5.)*r)
    
    dKdtheta[D - 1] = matern_5_2(x1, x2, params)
