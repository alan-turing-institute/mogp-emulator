...
void predict_single(
        ...
        bool calc_variance, bool calc_derivatives){
    ...
    auto square_exponential = cl::KernelFunctor<
        cl::Buffer, cl::Buffer, cl::Pipe&, cl::Pipe&, cl::Pipe&, cl::Buffer,
        float, int, int, int, int, int
        >(program, "sq_exp");
    // Create kernel functor for expectation kernel
    auto expectation = cl::KernelFunctor<
        cl::Pipe&, cl::Buffer, cl::Buffer, int, int
        >(program, "expectation");
    // Create kernel functor for variance kernel
    auto variance = cl::KernelFunctor<
        cl::Pipe&, cl::Buffer, cl::Buffer, float, int, int
        >(program, "variance");
    // Create kernel functor for derivative kernel
    auto derivatives = cl::KernelFunctor<
        cl::Pipe&, cl::Buffer, cl::Buffer, cl::Buffer, cl::Buffer, cl::Buffer,
        float, int, int, int
        >(program, "derivatives");
    ...
    cl_int status;
    cl_mem pipe_k = clCreatePipe(context(), 0, sizeof(cl_float), MAX_NX, NULL,
                                 &status);
    cl::Pipe k(pipe_k);
    cl_mem pipe_k2 = clCreatePipe(context(), 0, sizeof(cl_float), MAX_NX, NULL,
                                  &status);
    cl::Pipe k2(pipe_k2);
    cl_mem pipe_r = clCreatePipe(context(), 0, sizeof(cl_float),
                                 MAX_NX*MAX_NXSTAR, NULL, &status);
    cl::Pipe r(pipe_r);
    ...
    expectation(cl::EnqueueArgs(queue2, cl::NDRange(1)), k, d_InvQt, d_Ystar,
                nx, nxstar);
    if (calc_variance){
        variance(cl::EnqueueArgs(queue3, cl::NDRange(1)), k2, d_Ystarvar,
                 d_Q_chol, sigma, nx, nxstar);
    }
    if (calc_derivatives){
        derivatives(cl::EnqueueArgs(queue4, cl::NDRange(1)), r, d_Ystarderiv,
                    d_X, d_Xstar, d_InvQt, d_scale, sigma, nx, nxstar, dim);
    }
...
PYBIND11_MODULE(prediction, m){
    ...
    m.def("predict_single", &predict_single);
}
