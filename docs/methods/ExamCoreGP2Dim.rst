...
design from an :math:`p=2`-dimensional optimised Latin Hypercube in [0,1],
...
:math:`\strut n=30` points in :math:`[0,1]` are arranged in a :math:`(p \\times
n)` matrix which we call :math:`\strut D`.
...
:math:` c(x,x')=\exp\left[-\sum_{i=1}^p \\{(x_i -
...
:math:`\pi^*_{\tau}(\tau) \\propto (\widehat\sigma^2)^{-(n-q)/2}|A|^{-1/2}\|
H^\textrm{T} A^{-1} H|^{-1/2}\,.`

with

:math:`\widehat\sigma^2 = (n-q-2)^{-1}f(D)^\textrm{T}\left\{A^{-1} - A^{-1}
H\left( H^\textrm{T} A^{-1} H\right)^{-1}H^\textrm{T}A^{-1}\right\}
f(D)\,.`

:math:`\strut H` is the :math:`(n \\times p)` matrix
:math:`H=[h(x_1),h(x_2),\cdots,h(x_n)]^\textrm{T}`, with :math:`x_n` denoting
the :math:`\strut n`th design point.

:math:`\strut A` is the :math:`n\times n` correlation matrix of the design
...
:math:` c(x_i,x_j)=\exp\left[-\sum_{k=1}^p (x_{k,i} - x_{k,j})^2 /
\\exp(\tau_k)\right]`.

with :math:`x_{k,i}` denoting the :math:`\strut k`th input of the :math:`\strut
i`th design point. Recall also that in the above expressions the only
term that is a function of :math:`\tau` is the correlation matrix :math:`\strut
A`.
...
===================================================================================================
|image1|
Figure 2: Log posterior :math:`(\ln(\pi^*_{\tau}(\tau)))` as a function of :math:`\tau_1` and :math:`\tau_2`
===================================================================================================
...
:math:`\widehat\sigma^2=1.0290`

Finally, the regression coefficients :math:`\beta` are found with the
formula

:math:`\hat{\beta}=\left( H^\textrm{T} A^{-1} H\right)^{-1}H^\textrm{T}
A^{-1} f(D) \\,.`

are

:math:`\hat{\beta}=[33.5758, 4.9908, -39.7233]`
...
the validation set :math:`\strut D'`. The validation and the training
points are shown in figure 3.

=========================================================================================================
|image2|
Figure 3: The training (black crosses) and validation points (green stars) in the simulator's input space
=========================================================================================================
...
:math:`m^*(x) = h(x)^\textrm{T}\widehat\beta + c(x)^\textrm{T} A^{-1}
(f(D)-H\widehat\beta)`

and

:math:`v^*(x,x^\prime) = \\widehat\sigma^2\{c(x,x^\prime)\, -\,
...
points :math:`\strut D` and the validation points :math:`\strut D'`. That is,

:math:`D_{\textrm{new}} = \\{D,D'\}`
...
:math:`\hat{\delta} = [0.5437,0.0961]`

and by substitution we find

:math:`\widehat\sigma^2 = 0.9354`

:math:`\hat{\beta} = [33.5981, 4.8570,-39.6695]^{\mathrm{T}}`
...
We denote by :math:`\strut X` the model inputs, which are unknown, and we
assume that they follow a joint distribution :math:`\omega(X)`. We wish to
know what is the effect of this uncertainty about the inputs :math:`\strut
X` on the output :math:`f(X)`. In other words, we seek the mean
...
would involve drawing samples of :math:`\strut X` from :math:`\omega(X)` and
...
:math:`X_i \\sim {\cal N}(0.5,0.02)`, :math:`i \\in \\{1,2\}`

which yields for matrix :math:`\strut B`

:math:`B = \\left[\begin{array}{c c} 50 & 0 \\\0 & 50\end{array}\right]`

and

:math:`m = \\left[\begin{array}{c} 0.5 \\\0.5\end{array}\right]`
...
:math:`\tilde{X}_1 \\sim {\cal N}(1395,50)` for the solar constant, and

:math:`\tilde{X}_2 \\sim {\cal N}(0.4,0.0008)` for the albedo.

We should also mention that the matrix :math:`\strut C` is

:math:`C = \\left[\begin{array}{c c} \\delta_1^{-2} & 0 \\\0 &
\\delta_2^{-2}\end{array}\right] = \\left[\begin{array}{c c} 3.3828 & 0
\\\0 & 108.2812\end{array}\right]`

Also, since we have not used a nugget, we have :math:`\strut \\nu = 0`.
Finally, because both :math:`\strut B` and :math:`\strut C` are diagonal and
...
:math:`\textrm{E}^*[\textrm{E}[f(X)]] = 16.9857`

:math:`\textrm{Var}^*[\textrm{E}[f(X)]] = 0.0015`

:math:`\textrm{E}^*[\textrm{Var}[f(X)]] = 29.9588`
...
17.16 with a 95 :math:`:math:`% confidence interval of 0.11. The variance of the
...
================================================================================================================================================
|image4|
Figure 5: Uncertainty on the output due to :math:`\omega(X)`, as predicted by the emulator (continuous line) and by using Monte Carlo (dashed line)
================================================================================================================================================
...
:math:`\textrm{E}^*[V_w]`, with :math:`\strut w` being the set of indices of
...
=========================================================================================================================================================
|image5|
Figure 6: The average effects :math:`\textrm{E}^*[M_w(x_w)]` for :math:`\strut w=1` (solar constant, dashed line) and :math:`\strut w=2` (albedo, continuous line)
=========================================================================================================================================================

The values we calculated for the sensitivity indices are

:math:`\textrm{E}^*[V_1] = 0.54`

:math:`\textrm{E}^*[V_2] = 29.40`
...
about the model's output will decrease by approximately :math:`98:math:`%, while
learning the true value of the solar constant will decrease our
uncertainty only by the remaining :math:`2:math:`%.
...
==================================================================================================================================================================
|image6|
Figure 7: Simulator's output (mean surface temperature) as a function of the solar constant and the albedo. Black asterisks denote the design points :math:`\strut D`
==================================================================================================================================================================
...
===== ===== ===== ==== ==== ===== ===== ===== ===== ====
0.86  0.27  0.57  0.5  0.14 0.23  0.39  0.93  0.98  0.54
0.7   0.97  0.29  0.77 0.73 0.11  0.21  0.45  0.05  0.9
11.81 -4.95 25.51 5.27 4.85 30.28 27.50 20.89 34.50 0.43
===== ===== ===== ==== ==== ===== ===== ===== ===== ====
...
===== ==== ===== ===== ===== ===== ===== ===== ===== =====
0.00  0.83 0.16  0.37  0.76  0.64  0.58  0.91  0.42  0.21
0.12  0.83 0.51  0.64  0.44  0.70  0.46  0.03  0.24  0.94
28.66 4.68 15.04 11.63 20.39 10.83 18.78 34.76 26.60 -4.08
===== ==== ===== ===== ===== ===== ===== ===== ===== =====

Recall that the uncertainty and sensitivity analysis was carried out
using both data sets above as training data.

.. |image0| image:: images/ExamCoreGP2Dim/Figure1.png
   :width: 350px
   :height: 276px
.. |image1| image:: images/ExamCoreGP2Dim/Figure2.png
   :width: 350px
   :height: 276px
.. |image2| image:: images/ExamCoreGP2Dim/Figure3.png
   :width: 350px
   :height: 276px
.. |image3| image:: images/ExamCoreGP2Dim/Figure4.png
   :width: 350px
   :height: 276px
.. |image4| image:: images/ExamCoreGP2Dim/Figure5.png
   :width: 350px
   :height: 276px
.. |image5| image:: images/ExamCoreGP2Dim/Figure6.png
   :width: 350px
   :height: 276px
.. |image6| image:: images/ExamCoreGP2Dim/Figure7.png
   :width: 350px
   :height: 276px
