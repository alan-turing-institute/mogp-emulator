...
that result in differences between the simulator output :math:`\strut{ f }`
and the real system values :math:`\strut{ y }`. These are
...
still most likely not be exactly equal to the system :math:`\strut{y}`.
...
situation is by using three unknown functions :math:`\strut{ f}`,
:math:`\strut{f_{\rm theory}}` and :math:`\strut{f^+}` as follows:
...
(i.e. from :math:`\strut{ f}` to :math:`\strut{f_{\rm theory}}`) is relatively
...
:math:`\strut{f}` introduced in the move from (b) to (a), that is from
:math:`\strut{f_{\rm theory}}` to :math:`\strut{f^+}`, is far more complex and
...
might represent this as the improved model :math:`\strut{f'_{\rm theory}}`
which would lie between (a) and (b). Then we can consider the changes in
:math:`\strut{f}` when moving from :math:`\strut{f_{\rm theory}}` to
:math:`\strut{f'_{theory}}`, and from :math:`\strut{f'_{\rm theory}}` to
:math:`\strut{f^+}`.

It is now interesting to ask where the best input :math:`\strut{x^+}` and
the actual system :math::ref:`\strut{y}` might fit into this structure (see
`DiscBestInput<DiscBestInput>`). It can be shown (see Goldstein,
...
:math:`\strut{x^+}` (with corresponding independent model discrepancies)
into all the above models, and that a minimum description allowing
maximum flexibility is obtained by inserting :math:`\strut{x^+}` at the top
level, into what we define as the Reified model :math:`\strut{f^+}`, as is
discussed below.

Reified Modelling
~~~~~~~~~~~~~~~~~

Imagine constructing a reified model :math:`\strut{f^+}` which has both (i)
...
improvements to :math:`\strut{f^+}`. Therefore, the model discrepancy that
links :math:`\strut{f^+}` to the real system will be unstructured, for, by
...
Reified model; i.e. to :math:`\strut{f^+}` and to :math:`\strut{f^+}` alone,
in which case
...
:math::ref:`\strut{ \\perp }` to mean "independent of" (see
`DiscReificationTheory<DiscReificationTheory>` for more
details).
...
-  The simulator :math:`\strut{f}` is informative for :math:`\strut{y}`,
   because :math:`\strut{f}` is informative for the reified model
   :math:`\strut{f^+}`
...
If we have several simulators :math:`\strut{f_1, f_2, \\ldots, f_r}`, then
...
model :math:`\strut{f}`, with the Reified model :math:`\strut{f^+}`. Often we
would employ the use of an emulator to represent :math:`\strut{f}`, and
from this construct an emulator for :math:`\strut{f^+}`. As introduced
above, we would possibly consider emulators for intermediate models
:math:`\strut{f'}` to resolve specified deficiencies in our modelling. This
...
model discrepancy and helps bridge the gap between :math:`\strut{f}` and
:math:`\strut{f^+}`. The details of this process, and further discussion of
...
the emulator for :math:`\strut{f}`; see, Goldstein, M. and Rougier, J. C.
...
.. |image0| image:: /foswiki//pub/MUCM/MUCMToolkit/DiscReification/ReifNetwork1.png
   :width: 330px
   :height: 140px
